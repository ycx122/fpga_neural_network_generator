class FullyConnectedNetworkGenerator:
    ...
    def add_layer(self, input_width, input_num, output_width, output_num):
        layer_params = {
    ...
    def generate_top_module(self):
        ...
        for i, params in enumerate(self.layers):
            ...
            prev_instance_name = f"fc_layer{i}" if i > 0 else ""
            next_instance_name = f"fc_layer{i+2}" if i < len(self.layers) - 1 else ""
            
            instances.append(self.generate_fc_instance(params, f"fc_layer{i+1}", prev_instance_name, next_instance_name, weight_file))
            
            if i < len(self.layers) - 1:  # Add intermediate wires between layers
                wire_width = params['output_width'] * params['output_num']
                wires.append(f"wire [{wire_width-1}:0] data_out_fc_layer{i+1};\n")
...
network_generator.add_layer(8, 32, 16, 64)   # Adding first layer
network_generator.add_layer(16, 64, 16, 64)   # Adding first layer
network_generator.add_layer(16, 64, 8, 10)   # Adding second layer
